...
#ifndef MYNT_WALLET_WALLET_H
#define MYNT_WALLET_WALLET_H
...
public:
     ...
     * on this mynt node, and set to 0 for transactions that were created
...
public:
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins,
                            bool fGetMYNT = true, bool fOnlyAssets = false,
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Assets and MYNT
     */
