...
const std::string MyntGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MyntGUI::DEFAULT_WALLET = "~Default";

MyntGUI::MyntGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void MyntGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Mynt address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mynt: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to MYNT addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Mynt addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Mynt addresses"));
    ...
    openAction->setStatusTip(tr("Open a mynt: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Mynt command-line options").arg(tr(PACKAGE_NAME)));
...
void MyntGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/mynttext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* myntLabelLayout = new QVBoxLayout(toolbarWidget);
        myntLabelLayout->addWidget(label);
        myntLabelLayout->addWidget(toolbar);
        myntLabelLayout->setDirection(QBoxLayout::TopToBottom);
        myntLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Mynt Market Price"));
...
void MyntGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MyntGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MyntGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MyntGUI::setWalletActionsEnabled(bool enabled)
{
...
void MyntGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MyntGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MyntGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MyntGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MyntGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MyntGUI::openClicked()
{
...
void MyntGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MyntGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MyntGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MyntGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MyntGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MyntGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** MYNT START */
void MyntGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void MyntGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void MyntGUI::gotoManageAssetsPage()
{
...
void MyntGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Mynt network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MyntGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MyntGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MyntGUI::updateHeadersSyncProgressLabel()
{
...
void MyntGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MyntGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Mynt"); // default title
...
void MyntGUI::changeEvent(QEvent *e)
{
...
void MyntGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MyntGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MyntGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "MYNT")
        msg += tr("Amount: %1\n").arg(MyntUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MyntUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void MyntGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MyntGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MyntGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MyntGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MyntGUI::setHDStatus(int hdEnabled)
{
...
void MyntGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MyntGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MyntGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MyntGUI::detectShutdown()
{
...
void MyntGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MyntGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MyntGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MyntGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MyntGUI::subscribeToCoreSignals()
{
...
void MyntGUI::unsubscribeFromCoreSignals()
{
...
void MyntGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<MyntUnits::Unit> units = MyntUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MyntUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(MyntUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (MyntUnits::Unit u : MyntUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MyntUnits::name(u)), this);
...
{
    setText(MyntUnits::name(newUnits));
}
