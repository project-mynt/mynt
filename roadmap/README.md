...
Mynt (MYNT) is a Proof of Work coin built on the Bitcoin UTXO model. As with other Bitcoin derivatives, MYNT coins are distributed to persons augmenting the Mynt network by mining Mynt.
...
ASIC Resistance - A published commitment to continual attempts at ASIC resistance. If ASICs are created for x16r, then we will, at a specific block number, modify one of the algorithms to add some varients of Equihash or similar efforts to increase the resistance to ASIC miners for Mynt.

#### Asset Support

Mynt will be a hard fork that extends Mynt to include the ability to issue and transfer assets. The expected release of asset capabilities will be approximately seven months after the release of MYNT. Mynt will be extended to allow issuing, reissuing, and transfer of assets. Assets can be reissuable or limited to a set supply at the point of issuance. The cost to create assets will be 500 MYNT to create any qty of an asset. Each asset name must be unique. Asset names will be limited to A-Z and 0-9, '_' and '.' and must be at least three characters long. The '.' and the '_' cannot be the first, or the last character, or be consecutive.  
...
The MYNT used to issue assets will be sent to a burn address, which will reduce the amount of MYNT available.  

Asset transfers require the standard MYNT transaction fees for transfer from one address to another.
...
Reward capabilities will be added to allow payment (in MYNT) to all holders of an asset. Payments of MYNT would be distributed to all asset holders pro rata. This is useful for paying dividends, dividing payments, or rewarding a group of token holders.  

Example: A small software company issues an asset GAMECO that represents a share of the project. GAMECO tokens can be traded with others. Once the software company profits, those profits can be distributed to all holders of GAMECO by sending the profits (via MYNT) to all holders of GAMECO.

#### Block Size

Mynt may increase the blocksize from 2 MB to X MB to allow for more on-chain transactions.

### Phase 3 - Rewards

Rewards allow payment in MYNT to asset holders.

### Phase 4 - Unique Assets (Complete)

Once created, assets can be made unique for a cost of 5 MYNT. Only non-divisible assets can be made unique. This moves an asset to a UTXO and associates a unique identifier with the txid. From this point the asset can be moved from one address to another and can be traced back to its origin. Only the issuer of the original asset can make an asset unique.  
...
Voting will be accomplished by creating and distributing parallel tokens to token holders. These tokens can be sent to MYNT addresses to record a vote.

### Phase 7 - Compatibility Mode

Allows newly created assets to appear exactly like MYNT, LTC, or Bitcoin for easy integration into exchanges, wallets, explorers, etc.
