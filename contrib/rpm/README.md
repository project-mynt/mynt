...
The RPM spec file provided here is for Mynt-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://mynt.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Mynt git repository](https://github.com/project-mynt/Mynt) but are not part of
...
Currently only `Source10` is used, for the example `mynt.conf` file.
...
in the mynt git repository. At present only an SVG version of the Mynt
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the mynt
...
needed when 0.12.1 is released, a proper fix is already in the Mynt git
master branch.
...
idea to build Mynt Core against a static version of that library compiled
...
The `mynt.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `mynt-server` package. At this point, most RPM based
...
The `mynt-server` package should have SELinux support. How to properly do
...
Mynt-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/mynt/). He has been
packaging Mynt for Fedora at least since 2012.
...
4. I renamed the `mynt` package that contains the Qt GUI to `mynt-core` as
that appears to be how the general population refers to it, in contrast to
`mynt-xt` or `mynt-classic`. I wanted to make sure the general population
